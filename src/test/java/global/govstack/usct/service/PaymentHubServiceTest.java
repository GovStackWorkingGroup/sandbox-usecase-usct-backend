...
class PaymentHubServiceTest {
    ...
    @Mock
    private PaymentBBInformationMediatorProperties paymentBBInformationMediatorproperties;

    @Mock
    private PaymentHubBBInformationMediatorProperties paymentHubBBInformationMediatorProperties;

    @Mock
    private PaymentHubProperties paymentHubProperties ;
    ...
    public void setUp() {
        ...
        paymentHubService = new PaymentHubService(paymentHubProperties, paymentHubBBInformationMediatorProperties, requestFactory, beneficiaryRepository, paymentDisbursementRepository,packageService, restTemplate);
    }

    @Test
    void checkOrderPayment() {
        when(paymentHubProperties.bulkConnectorURL()).thenReturn("http://ph-ee-connector-bulk.paymenthub");
        when(restTemplate.postForObject(any(), any(), any())).thenReturn(null);
        var personDto = new CreatePersonDto("personalIdCode", "John", "Doe", "john.doe@email.com", "dateOfBirth", "test-region", "test-street", "1234567", "smith", "test-munic", "12345", "johndoe", "test-address", "BANK_ACCOUNT", "1234567891234567", "test-bank");
