...
package global.govstack.mocksris.util;
...
/**
 * A {@link Converter} that converts the provided {@link OAuth2UserRequest} to a {@link
 ...
 * @see RequestEntity
 */
public class OAuth2JwtUserRequestEntityConverter
    implements Converter<OAuth2UserRequest, RequestEntity<?>> {

  public static final MediaType APPLICATION_JWT = MediaType.valueOf("application/jwt");
  ...
  public RequestEntity<?> convert(OAuth2UserRequest userRequest) {
    ...
    headers.setAccept(Collections.singletonList(APPLICATION_JWT));
    URI uri =
    ...
    if (HttpMethod.POST.equals(httpMethod)) {
      ...
      formParameters.add(
          OAuth2ParameterNames.ACCESS_TOKEN, userRequest.getAccessToken().getTokenValue());
