...
public class PaymentHubService implements PaymentService {
    ...
    public PaymentHubService(PaymentHubProperties paymentHubProperties, HttpComponentsClientHttpRequestFactory requestFactory, BeneficiaryRepository beneficiaryRepository) {
    ...
    public void registerBeneficiary(List<Beneficiary> beneficiaries) {
        ...
        // httpHeaders.add("X-CallbackURL", paymentHubProperties.callbackBaseUrl() + "/api/v1/beneficiary-callback");
        httpHeaders.add("X-CallbackURL", "https://webhook.site/cf437b0e-06a3-4dbc-b994-10b61daf5704");
        httpHeaders.add("X-Registering-Institution-ID", paymentHubProperties.registeringInstitutionId());
    ...
    private PaymentHubOnboardingBeneficiaryDTO convertBeneficiaries(List<Beneficiary> beneficiaries, String requestID) {
        var paymentHubDetailsDto = beneficiaries.stream().map(beneficiary ->
    ...
    @Override
    @Transactional
    public void updatePaymentOnboardingStatus(List<String> failedFunctionalIds, String requestId, PaymentOnboardingCallbackMode onboardingMode) {
